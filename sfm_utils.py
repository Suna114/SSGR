...
def DeserializeKeypoints(kp): 
    ...
    out = []
    for point in kp:
        # OpenCV 4.x 兼容性：先创建KeyPoint对象，然后设置属性
        # 序列化格式: (pt, size, angle, response, octave, class_id)
        temp = cv2.KeyPoint()
        temp.pt = point[0]  # (x, y) 元组
        temp.size = float(point[1]) if point[1] is not None else 0.0
        temp.angle = float(point[2]) if point[2] is not None else -1.0
        temp.response = float(point[3]) if point[3] is not None else 0.0
        temp.octave = int(point[4]) if point[4] is not None else 0
        temp.class_id = int(point[5]) if point[5] is not None else -1
        out.append(temp)
